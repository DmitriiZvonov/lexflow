...
let recognition=null,listening=false,manualStop=true,textMode=false,mode='menu',quizTarget=0,quizLocked=false;
...
function showModuleView(){
 ...
 $('ivTrainerTitle').textContent=mode==='voice'?'Голосовой тренажёр':'Услышь и выбери форму';
...
function render(){const verb=currentVerb();if(!verb)return;$('ivRussian').textContent=verb.ru;$('ivCount').textContent=`${position+1} / ${order.length}`;$('ivProgressFill').style.width=`${((position+1)/order.length)*100}%`;$('ivHeard').textContent='';
 ...
 const revealed=solved[i]||peeked[i]||!settings.blurForms;if(solved[i])card.classList.add('iv-correct');else if(peeked[i])card.classList.add('iv-peek');else if(settings.blurForms)card.classList.add('iv-blurred');
...
function prepareQuiz(){quizLocked=false;solved=[false,false,false];peeked=[false,false,false];quizTarget=Math.floor(Math.random()*3);render();$('ivQuizStatus').textContent='Слушай слово и выбери, какая это форма';setTimeout(playQuizPrompt,350)}
...
function chooseQuiz(i){if(quizLocked)return;const card=$(`ivForm${i}`);if(i!==quizTarget){card.classList.add('iv-wrong');$('ivQuizStatus').textContent='Неверно. Попробуй ещё раз';signalError();setTimeout(()=>card.classList.remove('iv-wrong'),550);return}quizLocked=true;solved[i]=true;render();$('ivQuizStatus').textContent='Правильно ✓';setTimeout(()=>{if(position<order.length-1){position++;prepareQuiz()}else{mode='menu';showModuleView();window.showToast&&showToast('Упражнение завершено ✓')}},900)}
...
window.IrregularTrainer={open:openModule,close,backToMenu,startVoice,startQuiz,revealForm,speakForm,toggleListening,toggleTextMode,submitText,next:nextVerb,toggleSettings,updateSetting,updateQuizRate,playQuizPrompt};
})();
