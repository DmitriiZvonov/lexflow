...
function syncSettingsUI(){$('ivBlurToggle').checked=!!settings.blurForms;$('ivSoundToggle').checked=!!settings.errorSound;$('ivRandomToggle').checked=!!settings.randomOrder;const r=$('ivQuizRate'),n=$('ivQuizRateNum'),l=$('ivQuizRateValue');const v=Math.max(.1,Math.min(1,Number(settings.quizSpeechRate)||.72));settings.quizSpeechRate=v;if(r)r.value=v;if(n)n.value=v.toFixed(2);if(l)l.textContent=v.toFixed(2)+'×';paintQuizRate()}
...
window.IrregularTrainer={open:openModule,close,navigateBack,backToMenu,startVoice,startQuiz,revealForm,speakForm,toggleListening,toggleTextMode,submitText,next:nextVerb,toggleSettings,updateSetting,updateQuizRate,testQuizRate,playQuizPrompt};
})();
