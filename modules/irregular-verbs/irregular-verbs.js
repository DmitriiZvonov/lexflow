...
const defaults={blurForms:true,errorSound:true,randomOrder:false,quizSpeechRate:.72,quizRevealSeconds:4};
...
function syncSettingsUI(){$('ivBlurToggle').checked=!!settings.blurForms;$('ivSoundToggle').checked=!!settings.errorSound;$('ivRandomToggle').checked=!!settings.randomOrder;const r=$('ivQuizRate'),n=$('ivQuizRateNum'),l=$('ivQuizRateValue');const v=Math.max(.1,Math.min(1,Number(settings.quizSpeechRate)||.72));settings.quizSpeechRate=v;if(r)r.value=v;if(n)n.value=v.toFixed(2);if(l)l.textContent=v.toFixed(2)+'×';paintQuizRate();const rr=$('ivRevealTime'),rn=$('ivRevealTimeNum'),rl=$('ivRevealTimeValue');const rv=Math.max(1.5,Math.min(10,Number(settings.quizRevealSeconds)||4));settings.quizRevealSeconds=rv;if(rr)rr.value=rv;if(rn)rn.value=rv.toFixed(1);if(rl)rl.textContent=rv.toFixed(1)+' сек';paintRevealTime()}
...
function updateQuizRate(source){const r=$('ivQuizRate'),n=$('ivQuizRateNum'),l=$('ivQuizRateValue');if(!r||!n)return;let v=source==='number'?Number(n.value):Number(r.value);if(!Number.isFinite(v))return;v=Math.max(.1,Math.min(1,v));v=Math.round(v*100)/100;r.value=v;n.value=v.toFixed(2);settings.quizSpeechRate=v;if(l)l.textContent=v.toFixed(2)+'×';paintQuizRate();saveSettings()}
function testQuizRate(){window.LexFlowVoice&&LexFlowVoice.speak('became',{rate:Number(settings.quizSpeechRate)||.72})}
function paintRevealTime(){const r=$('ivRevealTime');if(!r)return;const min=Number(r.min),max=Number(r.max),v=Number(r.value),pct=((v-min)/(max-min))*100;r.style.background=`linear-gradient(90deg,var(--accent) 0 ${pct}%,var(--bg3) ${pct}% 100%)`}
function updateRevealTime(source){const r=$('ivRevealTime'),n=$('ivRevealTimeNum'),l=$('ivRevealTimeValue');if(!r||!n)return;let v=source==='number'?Number(n.value):Number(r.value);if(!Number.isFinite(v))return;v=Math.max(1.5,Math.min(10,v));v=Math.round(v*2)/2;r.value=v;n.value=v.toFixed(1);settings.quizRevealSeconds=v;if(l)l.textContent=v.toFixed(1)+' сек';paintRevealTime();saveSettings()}
...
window.IrregularTrainer={open:openModule,close,navigateBack,backToMenu,startVoice,startQuiz,revealForm,speakForm,toggleListening,toggleTextMode,submitText,next:nextVerb,toggleSettings,updateSetting,updateQuizRate,testQuizRate,updateRevealTime,playQuizPrompt};
})();
